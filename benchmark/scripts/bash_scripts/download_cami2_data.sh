...
if [ ! -d $output_dir/CAMI_data/rhizosphere_data ]; then
    ...
    parallel -j $cpu_num wget -P $output_dir/CAMI_data/rhizosphere_data https://frl.publisso.de/data/frl:6425521/plant_associated/short_read/rhimgCAMI2_sample_{}.tar.gz ::: {0..20};
    ## Extract the fq.gz files
    cd $output_dir/CAMI_data/rhizosphere_data;
...
if [ ! -d $output_dir/CAMI_data/marine_data ]; then
    ...
    ## Extract the fq.gz files
    cd $output_dir/CAMI_data/marine_data;
    ## backup the raw files
    if [ ! -d ./backup ]; then
        mkdir -p ./backup;
    fi
...
if [ ! -d $output_dir/CAMI_data/strain_madness_data ]; then
    ...
    ## Extract the fq.gz files
    cd $output_dir/CAMI_data/strain_madness_data;
    ## backup the raw files
    if [ ! -d ./backup ]; then
        mkdir -p ./backup;
    fi
