...
if [ $# -eq 0 ]; then
    echo "Usage: run_YACHT.sh <yacht_repo_loc> <output_dir> <cpu_num>"
    exit 1
fi
yacht_repo_loc=$1
output_dir=$2
...
if [ ! -d $yacht_repo_loc/gtdb_reference ]; then
    ...
    weget -P $yacht_repo_loc/gtdb_reference https://data.gtdb.ecogenomic.org/releases/release214/214.1/ar53_metadata_r214.tar.gz;
...
if [ ! -d $output_dir/results/YACHT_results/rhizosphere_data ]; then
    ...
    if [ ! -d $output_dir/CAMI_data/rhizosphere_data/sketches ]; then
        ...
        cd $output_dir/CAMI_data/rhizosphere_data/sketches;
        parallel -j $cpu_num sourmash sketch dna -f -p k=31,scaled=1000,abund -o rhimgCAMI2_sample_{}.sig.zip $output_dir/CAMI_data/rhizosphere_data/rhimgCAMI2_sample_{}.fq.gz ::: {0..20};
    fi
...
if [ ! -d $output_dir/results/YACHT_results/pathogen_detection_data ]; then
    ...
    if [ ! -d $output_dir/CAMI_data/pathogen_detection_data/sketches ]; then
        ...
        cd $output_dir/CAMI_data/pathogen_detection_data/sketches;
        sourmash sketch dna -f -p k=31,scaled=1000,abund -o patmgCAMI2.sig.zip $output_dir/CAMI_data/pathogen_detection_data/*.fastq.gz;
    fi
    ## run YACHT on the samples
...
if [ ! -d $output_dir/results/YACHT_results/marine_data ]; then
    ...
    if [ ! -d $output_dir/CAMI_data/marine_data/sketches ]; then
        ...
        cd $output_dir/CAMI_data/marine_data/sketches;
        parallel -j $cpu_num sourmash sketch dna -f -p k=31,scaled=1000,abund -o marmgCAMI2_sample_{}.sig.zip $output_dir/CAMI_data/marine_data/marmgCAMI2_sample_{}.fq.gz ::: {0..9};
    fi
    ## run YACHT on the samples
...
if [ ! -d $output_dir/results/YACHT_results/strain_madness_data ]; then
    ...
    if [ ! -d $output_dir/CAMI_data/strain_madness_data/sketches ]; then
        ...
        cd $output_dir/CAMI_data/strain_madness_data/sketches;
        parallel -j $cpu_num sourmash sketch dna -f -p k=31,scaled=1000,abund -o strmgCAMI2_sample_{}.sig.zip $output_dir/CAMI_data/strain_madness_data/strmgCAMI2_sample_{}.fq.gz ::: {0..99};
    fi
    ## run YACHT on the samples
